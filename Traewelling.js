...
const Cache = importModule('/lib/cache.js');
const Updater = importModule('/lib/updater.js'); 
const http = importModule('/lib/http.js');

const scriptVersion = 1;
const sourceRepo = "tiuub/traewelling-widget";
const sourceRepoUrl = "https://github.com/" + sourceRepo;
const scriptName = "Traewelling";
const scriptAuthor = "tiuub"
const scriptLicense = "Apache 2.0"
...
try {
  areRepoUpdatesAvailable = await updater.checkForUpdate(scriptName, scriptVersion);
} catch (error) {
...
if (areRepoUpdatesAvailable) {
  try {
    await updater.updateScript(scriptName, scriptVersion);
  } catch (error) {
    console.log(`${error.line}: ${error.message}`);
  }
}
...
async function createWidget() {
  ...
  if (areRepoUpdatesAvailable)
    widget.url = sourceRepoUrl + "/releases/latest"
  else
  ...
  data = await getStatisticsDailyByDayDiff(apiKey, days)
...
async function addStationsCount(widget, data) {
  ...
  t_trainstations = widget.addText(`🏫 ${trainstations.length} stations`)
...
async function addFavouriteStation(widget, data) {
  ...
  t_favouritestation = widget.addText(`⭐️ ${evaToNames[highestEva]} (${stationCount[highestEva]})`)
...
async function addTrainDelay(widget, data) {
  ...
  t_delay = widget.addText(`⏳ ${trainDelayHours}h ${trainDelayMinutes}min delay`)
...
async function addTrainAvgSpeed(widget, data) {
  ...
  sumDurations = sum(speedsDurations.map(dict => dict["duration"]))
  let trainAvgSpeed = 0
  ...
  t_avgspeed = widget.addText(`🚄 ${numberWithCommas(trainAvgSpeed, 0)} km/h (avg)`)
...
async function addPercentageByDuration(widget, trips, key, displayFunc) {
  ...
  t_purpose = widget.addText(displayFunc(percentages))
...
async function addFastestTrips(widget, trips, maxTrips=7) {
  ...
  let displayFunc = function(trip) { return `(${numberWithCommas(trip.train.speed, 0)} km/h)` }
...
async function addLongestTimeTrips(widget, trips, maxTrips=7) {
  ...
  let displayFunc = function(trip) { return `(${minutesToHoursMinutesString(trip.train.duration)})` }
...
async function addHighestDelayTrips(widget, trips, maxTrips=7) {
  ...
  let displayFunc = function(trip) { return `(${minutesToHoursMinutesString((new Date(trip.train.destination.arrivalReal) - new Date(trip.train.destination.arrivalPlanned)) / 60000)})` }
...
async function addRecordTrips(widget, title, trips, maxTrips=7, displayParameterFunc= (function() { return "" })) {
  ...
  if (trips.length <= 0) {
    ...
    t_trip = widget.addText("😿 No trips")
  ...
  for (let i = 0; i < l; i++) {
    ...
    t_trip = widget.addText(`${train_emoji(trainCategory)} ${displayParameter} ${trainOriginName} to ${trainDestinationName} ~ ${numberWithCommas(trainDistanceInKm, 0)} km ${purposeEmoji}`)
    t_trip.font = Font.regularSystemFont(12)
    t_trip.textColor = Color.white()
  }
}
...
async function getStatisticsDailyByDayDiff(apiKey, days=7) {
  ...
  for (let i = 0; i < days; i++) { 
    ...
    j = await loadStatisticsDaily(apiKey, date, cacheExpiration)
...
async function loadStatisticsDaily(apiKey, date, cacheExpiration=0) {
  ...
  let json = await http.fetchJson(
      ...
      headers,
      cache,
      `traewelling_personal_${username}_${dateString}`,
      cacheExpiration
    );
  return json
}
